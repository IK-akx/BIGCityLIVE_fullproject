...
exports.createResource = async (req, res, next) => {
  try {
    ...
    if (!type) return res.status(400).json({ message: "Ticket type not found for this event" });
    ...
    // store price at purchase time (important!)
...
exports.getResourceById = async (req, res, next) => {
  try {
    ...
    const ticket = await Ticket.findById(id).populate("event", "title date location category");
    if (!ticket) return res.status(404).json({ message: "Resource not found" });
...
exports.updateResourceById = async (req, res, next) => {
  try {
    const userId = req.user.userId;
    const { id } = req.params;
    ...
    if (status !== "cancelled") {
      return res.status(400).json({ message: "Only status='cancelled' is supported in update" });
    }
...
exports.deleteResourceById = async (req, res, next) => {
  try {
    ...
    // if active -> restore inventory before deleting
